...
export default function AddReceivableModal({ onClose, onAdded, onUpdated, editData }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-[#1C1915] border border-[#2C2922] rounded-2xl w-full max-w-md mx-4 p-6" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs text-gray-400 mb-1">{t("common.amount")}</label>
          ...
          <div>
            <label className="block text-xs text-gray-400 mb-1">{t("common.date")} ({t("common.optional")})</label>
