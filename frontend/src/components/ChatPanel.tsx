...
export default function ChatPanel({ initialInsight }: Props) {
  ...
  const doSend = async (text: string) => {
    ...
    try {
      ...
      if (result.profile_updated) showToast(t("chat.profileUpdated"));
      if (result.pending_transaction) setPendingTx(result.pending_transaction);
    } catch {
      setMessages((prev) => [
        ...prev,
        { id: `e-${Date.now()}`, role: "assistant", content: t("common.error"), sentAt: new Date().toISOString() },
      ]);
    } finally {
      setSending(false);
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          id: `tx-${Date.now()}`,
          role: "assistant",
          content: `✓ ${t("chat.addTx")}: ${formatAmount(pendingTx.amount)} — ${pendingTx.category}`,
          sentAt: new Date().toISOString(),
        },
      ]);
      setPendingTx(null);
    } catch (err) {
      setTxError(err instanceof Error ? err.message : t("common.error"));
    } finally {
  ...
  const typeLabel = pendingTx?.type === "credit" ? t("progress.income") : t("progress.spending");
  const catKey = pendingTx ? `category.${pendingTx.category}` : "";
  const catLabel = pendingTx ? (t(catKey) !== catKey ? t(catKey) : pendingTx.category) : "";
      ...
      <div className="rounded-xl bg-[#1A1A1A] border border-[#2A2A2A] flex flex-col mb-8" style={{ height: 420 }}>
        <div className="px-4 py-3 border-b border-[#2A2A2A] flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2">
            ...
            <p className="text-xs text-gray-300 font-semibold tracking-wide">{t("chat.title")}</p>
          </div>
          {toast && <span className="text-xs text-emerald-400 font-medium animate-pulse">{toast}</span>}
        </div>

        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3 min-h-0">
          {!historyLoaded && (
            <p className="text-gray-600 text-sm text-center py-6 animate-pulse">{t("common.loading")}</p>
          )}
          {historyLoaded && messages.length === 0 && (
            <p className="text-gray-600 text-sm text-center py-6">{t("chat.empty")}</p>
          )}
          ...
          {messages.map((msg) => {
            ...
            return (
              <div key={msg.id} className={`flex flex-col gap-0.5 ${msg.role === "user" ? "items-end" : "items-start"}`}>
                ...
                {timeStr && <span className="text-[10px] text-gray-700 px-1">{timeStr}</span>}
              </div>
            );
          })}
          ...
          {pendingTx && !sending && (
            <div className="flex justify-start">
              <div className="max-w-[90%] bg-[#1A1A1A] border border-indigo-800/60 rounded-2xl rounded-bl-sm px-4 py-3 space-y-2">
                <p className="text-xs text-gray-500 uppercase tracking-wide font-semibold">{t("chat.addTx")}</p>
                <p className="text-sm text-white font-medium">
                  {formatAmount(pendingTx.amount)}{" "}
                  <span className="text-gray-400 font-normal">{typeLabel} · {catLabel}</span>
                </p>
                ...
                <div className="flex gap-2 pt-1">
                  ...
                  >
                    {txConfirming ? t("common.loading") : t("chat.confirmTx")}
                  </button>
                  <button
        ...
        <div className="px-4 py-3 border-t border-[#2A2A2A] flex gap-2 items-end shrink-0">
          {voiceSupported && (
            <button
              ...
              disabled={sending}
              title={recording ? t("chat.listening") : t("chat.voiceInput")}
              className={`shrink-0 w-9 h-9 rounded-xl flex items-center justify-center transition-colors disabled:opacity-40 ${
                recording ? "bg-red-600 hover:bg-red-500" : "bg-[#2A2A2A] hover:bg-[#333] text-gray-400"
              }`}
            >
              {recording ? (
