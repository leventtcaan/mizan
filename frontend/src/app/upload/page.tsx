...
export default function UploadPage() {
  ...
  // Free-tier flag for the vision-parsing upsell moments (scanned PDFs the free
  // engine can't read are exactly what paid AI vision is for).
  const isFree = (getStoredUser()?.plan ?? "free") === "free";
  // Business accounts get business framing (statements, invoice exports, expense reports).
  const isBusiness = getStoredUser()?.account_type === "business";
  const bt = (key: string): string => {
    if (!isBusiness) return t(key);
    const v = t(`${key}Biz`);
    return v !== `${key}Biz` ? v : t(key);
  };
  ...
  if (processing) {
    return (
      <PageLayout title={bt("upload.title")} subtitle={bt("upload.subtitle")} maxWidth="sm">
          ...
          {entries.map((e, i) => (
            <div key={`${e.file.name}-${i}`} className="flex items-center gap-3 p-3 rounded-lg bg-surface border border-line">
              ...
              <div className="flex-1 min-w-0">
                ...
                {/* The honest conversion moment: a scanned PDF the free engine couldn't
                    read is EXACTLY what paid AI vision parsing is for. */}
                {isFree && e.state === "done" && e.result && e.result.status !== "success" &&
                  (e.result.reason === "scanned_image" || e.result.reason === "ocr_unavailable") && (
                  <Link href="/upgrade" className="inline-flex items-center gap-1 mt-1 text-[#176B5B] text-xs font-medium hover:underline">
                    <Sparkles size={12} /> {t("upload.visionUpsell")}
                  </Link>
                )}
              </div>
