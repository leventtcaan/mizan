...
export default function InstallmentsPage() {
  ...
  return (
    <PageLayout title="Taksitler" subtitle="Tespit edilen taksit planları ve gerçek maliyet">
      ...
      {!loading && plans.length === 0 && (
        <div className="text-center py-20 bg-[#1A1A1A] border border-[#2A2A2A] rounded-xl text-gray-500">
          <p className="text-base text-gray-400">Taksit planı tespit edilmedi.</p>
        ...
        <>
          ...
          <div className="bg-[#1A1A1A] border border-[#2A2A2A] rounded-xl p-6 mb-6">
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
              <div>
                <p className="text-gray-500 text-sm mb-2">Aylık taksit yükümlülüğünüz</p>
                <p className="text-5xl font-bold text-white">₺{fmtN(summary?.total_monthly_burden ?? 0)}</p>
                <p className="text-gray-500 text-sm mt-2">{plans.length} aktif plan</p>
              </div>
            ...
            {summary && summary.total_opportunity_loss > 0 && (
              <div className="mt-5 pt-5 border-t border-[#2A2A2A]">
                <p className="text-gray-500 text-xs">
                  Fırsat maliyeti (kalan taksitler, %40 yıllık getiri varsayımı):
            ...
            {plans.map((plan) => {
              ...
              return (
                <div key={plan.merchant_key} className="bg-[#1A1A1A] border border-[#2A2A2A] rounded-xl p-5">
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex gap-4 text-xs text-gray-500 flex-wrap">
                        <span>İlk: {new Date(plan.first_seen).toLocaleDateString("tr-TR", { month: "short", year: "numeric" })}</span>
                  ...
                  <div className="mb-4">
                    <ProgressBar current={plan.months_detected} total={plan.total_plan_months} />
                  </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-4">
                    ...
                    <div className="bg-amber-950/30 border border-amber-900/20 rounded-lg p-3">
                      ...
                      <p className="text-amber-800 text-xs">fırsat maliyetiyle</p>
                    </div>
                  </div>
                  ...
                  {plan.estimated_remaining > 0 && (
                    <div>
                      ...
                      >
                        ...
                        Erken ödeyebilirim — ne kadar tasarruf ederim?
                      </button>

                      {earlyOpen && (
                        <div className="mt-3 bg-indigo-950/20 border border-indigo-900/30 rounded-xl p-4">
                          <p className="text-indigo-300 text-sm font-medium mb-3">Erken Ödeme Analizi</p>
                          ...
                          <p className="text-indigo-500 text-xs mt-3">
                            %40 yıllık getiri varsayımıyla. Bankanızın erken ödeme koşullarını kontrol edin.
                          </p>
                        </div>
                      )}
                    </div>
