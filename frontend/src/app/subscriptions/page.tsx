...
function fmt(amount: string): string {
  return new Intl.NumberFormat("tr-TR", { minimumFractionDigits: 0, maximumFractionDigits: 0 }).format(parseFloat(amount));
}

function fmt2(amount: string): string {
  return new Intl.NumberFormat("tr-TR", { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(parseFloat(amount));
}
...
export default function SubscriptionsPage() {
  ...
  return (
    <PageLayout title="Abonelikler" subtitle="Tespit edilen tekrarlayan ödemeler">
      ...
      {!loading && subs.length === 0 && (
        <div className="text-center py-20 text-gray-500 bg-[#1A1A1A] border border-[#2A2A2A] rounded-xl">
          <p className="text-base text-gray-400">Henüz tekrarlayan ödeme tespit edilmedi.</p>
        ...
        <>
          ...
          <div className="bg-[#1A1A1A] border border-[#2A2A2A] rounded-xl p-6 mb-6">
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-5">
              <div>
                <p className="text-gray-500 text-sm mb-2">Aylık abonelik harcaman</p>
                <p className={`text-5xl font-bold ${totalMonthly > 500 ? "text-red-400" : "text-white"}`}>
                  ₺{fmt(String(totalMonthly))}
                </p>
                <p className="text-gray-500 text-sm mt-2">{summary?.count ?? 0} aktif abonelik</p>
              </div>
              {potentialSavings > 0 && (
                <div className="bg-emerald-950 border border-emerald-800 rounded-xl px-5 py-4">
                  <p className="text-emerald-500 text-xs mb-1">Tasarruf potansiyeli</p>
                  <p className="text-emerald-300 text-2xl font-bold">₺{fmt(String(potentialSavings))}</p>
                  <p className="text-emerald-700 text-xs mt-0.5">/ay</p>
                </div>
              )}
            </div>
          </div>
          ...
          {/* Active subs */}
          <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-3">Aktif Abonelikler</h2>
                ...
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex gap-4 text-xs text-gray-500 flex-wrap">
                      <span>Son: {new Date(sub.last_seen).toLocaleDateString("tr-TR", { day: "numeric", month: "short", year: "numeric" })}</span>
                      <span>{sub.months_active} ay aktif</span>
                      <span>Toplam: ₺{fmt2(sub.total_paid_all_time)}</span>
                    </div>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="text-2xl font-bold text-white">₺{fmt2(sub.avg_amount)}</p>
                    <p className="text-xs text-gray-500">/{sub.frequency === "weekly" ? "hafta" : "ay"}</p>
                  </div>
                </div>
          ...
          {reviewSubs.length > 0 && (
            <div className="bg-amber-950/20 border border-amber-800/30 rounded-xl p-5 mb-6">
              <h2 className="text-amber-300 font-semibold mb-3 text-sm">Gözden Geçirilecekler</h2>
              ...
              <div className="mt-4 pt-3 border-t border-amber-800/30 flex justify-between items-center">
                <span className="text-gray-400 text-sm">Toplam tasarruf potansiyeli</span>
                <span className="text-emerald-400 font-bold">₺{fmt(String(potentialSavings))}/ay</span>
              </div>
            </div>
          )}
          ...
          {cancelledSubs.length > 0 && (
            <div>
              <h2 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">İptal Edilenler</h2>
              <div className="space-y-2">
