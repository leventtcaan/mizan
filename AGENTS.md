...
**Phases 1–98 complete. Alembic head = 0038.** (Older Phase 42/head-0022 notes below are historical — CLAUDE.md is authoritative.) Shipped since last update: design system app-wide (82–91, 95–97), auth/onboarding redesign (92), registration consent fields **migration 0037** (93), statement bridge ekstre→varlık **migration 0038** (94), Brief redesign (96), DB-backed free-tier assistant cap (98). **State: design system established (light default, teal #176B5B, terracotta negatives); email verification + Redis limiter + free/plus/pro plan + upload cap + consent capture + statement bridge live. Pending: (1) deploy** — Railway + Vercel; **SECRET_KEY + RESEND_API_KEY + REDIS_URL** via env; docker-compose has Redis; `alembic upgrade head` (→0038) on cold start. **(2) Resend domain verification.** **(3) Stripe billing** — `/upgrade` interest-only; wire checkout → webhook sets `plan`/`plan_expires_at`. **(4) Statement bridge live testing** (ekstre↔varlık match across banks/formats). **⚠ Untraced**: solid `bg-<token>` utilities don't paint reliably (mitigated with inline colors on overlays/selected fills). **Hold the line on SUBTRACTION.**
