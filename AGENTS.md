...
**Phases 1–105 complete. Alembic head = 0042.** Since 98: admin command-center (99); liability living-obligation + proactive Mim triggers, migrations 0039–0041 (100); user profile + business fields, 0042 (101); settings read-only redesign (102); security fixes — Next 14.2.35, /chat removed, LLM provider call fix, plan gates (get_paid_user), email normalize (103); statement bridge multilingual date-aware balance, Itaú/Brazilian (104); reports period picker + date-range filenames + full CSV/Excel (105). **Pending (audit): account-ownership validation (networth.py), category-list unification (egitim drift), cross-batch dedup (full desc), upgrade honest copy, hardcoded-TR warnings, reports currency default; + automated tests, Stripe billing, deploy.** (CLAUDE.md is authoritative for detail.) **Design: light default, teal #176B5B primary (literal hex — `bg-<token>` solid fills have an untraced runtime paint quirk, so overlays + selected fills use inline colors), terracotta #B54747 negatives. Pricing: Free ₺0 · Plus ₺199/ay·₺1.690/yıl ($7/$59) · Pro ₺349/ay·₺2.990/yıl ($12/$99) — billing NOT integrated, `/upgrade` captures interest only. IMPORTANT: cash flow↔net worth bridge (ekstre→varlık) IMPLEMENTED Phase 94 (statement_bridge.py — balance detect + asset/liability propose→confirm); ekstre↔varlık auto-match wired but needs live testing.** **Vision: chief of staff, not dashboard. Brief + Simulator = soul; dashboards = doorways. One sentence + everything behind a tap; subtraction > addition.** Home (69) is now a one-sentence coach. Most of 65–69 verified by build/py_compile only (no live LLM key here). Loop: Upload → Review → Brief → Home (Phases 61–64). Brief reuses ProgressInsight `data_type="brief"` (no new table). Progress = Financial Health scorecard. Net Worth = GuidancePanel + AllocationChart (no history chart). Upload returns status/reason; parser is global. Deferred: `_generate_networth_suggestions` Turkish bank keywords; account connectivity (Plaid — deferred, manual-first chosen); CSV unquoted comma-thousands edge case; scorecard synthetic score when thin; real snapshots need time (trajectory estimated until then); P1-deep valuation migration; P2 tx↔account reconciliation.
...
**Phases 1–105 complete. Alembic head = 0042.** (Older Phase 42/head-0022 notes below are historical — CLAUDE.md is authoritative.) Shipped since 98: admin command-center (99); liability living-obligation + proactive Mim triggers, **migrations 0039–0041** (100); user profile + business fields, **0042** (101); settings read-only redesign (102); security fixes — Next 14.2.35, /chat removed, LLM provider call fix, plan gates, email normalize (103); statement bridge multilingual date-aware balance (104); reports period picker + full CSV/Excel (105). **Pending — audit fixes FIRST: (1) account-ownership validation (networth.py account_id), (2) unify category list (egitim drift across upload/categorizer/transactions), (3) cross-batch dedup use full description not [:30], (4) upgrade honest copy, (5) hardcoded-TR net-worth warning → use lang, (6) reports default currency → user display_currency. Then: automated tests, Stripe billing, deploy** (Railway + Vercel; SECRET_KEY + RESEND_API_KEY + REDIS_URL; `alembic upgrade head` →0042; Resend domain verify). **⚠ Untraced**: solid `bg-<token>` utilities don't paint reliably (mitigated with inline colors on overlays/selected fills). **Hold the line on SUBTRACTION.**
